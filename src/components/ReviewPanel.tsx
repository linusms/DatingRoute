  ...
  return (
    <div style={{
      ...
      >
        ...
        >
          <div style={{ width: '40px', height: '3px', borderRadius: '2px', background: 'var(--color-border)' }} />
        </div>
        ...
        <div className="review-header" style={{
          ...
          <div>
            <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--color-text-primary)', marginBottom: '4px' }}>{placeName}</h2>
            <div style={{ fontSize: '13px', color: 'var(--color-text-secondary)' }}>리뷰 및 후기</div>
          </div>
        ...
        {/* Tabs */}
        <div className="review-tabs" style={{ display: 'flex', borderBottom: '1px solid var(--color-border)', flexShrink: 0 }}>
          <button
            style={{
              flex: 1, padding: '12px', background: 'transparent', border: 'none',
              color: activeTab === 'youtube' ? 'var(--color-accent-primary)' : 'var(--color-text-secondary)',
              borderBottom: activeTab === 'youtube' ? '2px solid var(--color-accent-primary)' : '2px solid transparent',
          ...
          <button
            style={{
              flex: 1, padding: '12px', background: 'transparent', border: 'none',
              color: activeTab === 'blog' ? 'var(--color-accent-primary)' : 'var(--color-text-secondary)',
              borderBottom: activeTab === 'blog' ? '2px solid var(--color-accent-primary)' : '2px solid transparent',
        ...
        <div className="review-content custom-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
          {isLoading && <div style={{ color: 'var(--color-text-secondary)', textAlign: 'center', padding: '40px' }}>불러오는 중...</div>}
          {error && <div style={{ color: '#ef4444', textAlign: 'center', padding: '40px' }}>{error}</div>}

          {!isLoading && !error && activeTab === 'youtube' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {videos.length === 0 ? (
                <div style={{ color: 'var(--color-text-secondary)', textAlign: 'center', padding: '40px' }}>관련 영상이 없습니다.</div>
              ) : (
                <>
                  <div className="ai-recommendation animate-fade-in" style={{
                    ...
                    <strong style={{ color: 'var(--color-accent-primary)', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px', fontSize: '15px' }}>
                      <span>✨</span> Gemini 유튜브 핵심 요약
                    </strong>
                    {ytSummary ? (
                      <div style={{ whiteSpace: 'pre-wrap' }}>{ytSummary}</div>
                    ) : (
                      <div style={{ color: 'var(--color-text-secondary)' }}>요약을 생성하는 중입니다...</div>
                    )}
                  </div>
                  <h4 style={{ color: 'var(--color-text-secondary)', marginTop: '8px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>참고한 영상 목록</h4>
                  {videos.map((vid) => (
                  <a key={vid.id} href={vid.url} target="_blank" rel="noreferrer" style={{
                    display: 'flex', gap: '12px', textDecoration: 'none', background: 'var(--color-bg-secondary)',
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      <div style={{ fontSize: '14px', color: 'var(--color-text-primary)', fontWeight: 500, marginBottom: '6px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{stripHtml(vid.title)}</div>
                      <div style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>{vid.channelTitle}</div>
                    </div>
                  </a>
                ))}
              </>
          ...
          {!isLoading && !error && activeTab === 'blog' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {blogs.length === 0 ? (
                <div style={{ color: 'var(--color-text-secondary)', textAlign: 'center', padding: '40px' }}>관련 블로그가 없습니다.</div>
              ) : (
                <>
                  <div className="ai-recommendation animate-fade-in" style={{
                    background: 'var(--color-border)',
                  ...
                  <h4 style={{ color: 'var(--color-text-secondary)', marginTop: '8px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>참고한 블로그 목록</h4>
                  {blogs.map((blog, i) => (
                    <a key={i} href={blog.link} target="_blank" rel="noreferrer" style={{
                      display: 'block', textDecoration: 'none', background: 'var(--color-bg-secondary)',
                      padding: '16px', borderRadius: '12px', border: '1px solid var(--color-border)'
                    }}>
                      <div style={{ fontSize: '14px', color: 'var(--color-text-primary)', fontWeight: 600, marginBottom: '8px' }}>
                        {stripHtml(blog.title)}
                      </div>
                      <div style={{ fontSize: '12px', color: 'var(--color-text-secondary)', lineHeight: 1.5, marginBottom: '8px', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {stripHtml(blog.description)}
                      </div>
                      <div style={{ fontSize: '11px', color: 'var(--color-accent-primary)' }}>{blog.postdate}</div>
                    </a>
                  ))}
                </>
              )}
